...
export function NotebookEntry({ record: supplied, recordId, chapters, className = '', introduction }: {
 ...
 const manuscript = <Manuscript introduction={<><p>{record.dek}</p><p className="notebook-manuscript-state">{record.publication === 'published' ? 'Published manuscript' : 'Working manuscript'} · v{record.version}</p></>}>
   ...
   <footer className="manuscript-end"><a href={`#${folios[0].id}`}>{format === 'lookbook' ? 'Return to the exhibition' : 'Return to the spreads'} ↗</a><a href="#cite">Sources and citation ↗</a></footer>
  </Manuscript>;
 if (format === 'lookbook') return <NotebookExhibition id={id} title={record.title} subtitle={record.dek}
  byline={<><span>{record.authors.join(', ')}</span><time dateTime={record.published || record.created}>{record.published || record.created}</time><span>{record.publication === 'published' ? 'Published' : 'Working draft'} · v{record.version}</span></>}
  sections={sections.map((section, index) => ({ id: section.id || `${id}-folio-${index + 1}`, label: section.label, children: <div className={`notebook-exhibition-content ${className}`}>{section.children}</div> }))}
