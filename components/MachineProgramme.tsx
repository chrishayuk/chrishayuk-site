...
export function MachineJourney({ id }: { id: string }) {
  return <nav className="machine-journey" aria-label="Machine experiments reading journey">
    <div className="record-voice"><Link href="/thread/machines">MACHINE EXPERIMENTS / FOLLOW THE QUESTION ↗</Link><span>FOUR CONNECTED NOTES</span></div>
...
export function MachineInstruments() {
  return <section id="instruments" className="machine-instruments"><p className="record-voice">THE EXPERIMENTS AND THE PLACES AROUND THEM</p><h2>Different instruments.<br/><em>Different kinds of evidence.</em></h2><div>{[
    ...
    {href:"https://llmwilds.fly.dev",label:"04 / THE HABITAT",title:"LLM Wilds",text:"The separate site used for Authority-2. Its current contents may differ from the closed experiment."},
...
export function MachineHomeFeature() {
  return <section id="machine-experiments" className="machine-home-feature" aria-labelledby="machine-home-heading" data-hause-act="connection">
    <div className="machine-home-heading"><div><p className="kicker record-voice">MACHINE GUESTBOOK / A LIVE INSTRUMENT, FOUR WORKING NOTES</p><h2 id="machine-home-heading">When a website asks<br/>an AI agent to act,<br/><em>whose instruction counts?</em></h2></div><div><p>Agents visited this publication, tried its invitations and sometimes found the experiment itself. Each encounter changed the next question.</p><p>The latest completed comparison moved to LLM Wilds. The website offered the same recording mechanism; the assigned task changed what happened.</p><div className="inline-links"><Link className="text-link" href="/notebook/the-page-could-ask-it-couldnt-authorise">READ THE LATEST EXPERIMENT ↗</Link><Link className="text-link" href="/thread/machines">FOLLOW ALL FOUR NOTES ↗</Link></div></div></div>
    <MachineOutcomeStrip/>
