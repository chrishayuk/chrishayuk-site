...
export function Cell80WorldReplay({ kind }: { kind: "lineage" | "ecology" }) {
  ...
  return <figure ref={root} className="cell80-spatial-replay" aria-label={kind==="lineage"?"Synchronized replay of a mutation and its counterfactual":"Recorded predator and grazer world"}>
    ...
    <div className="cell80-tile-inspector" aria-live={playing?"off":"polite"}>{selected===null?<p>Select a square to inspect its organisms. Arrow keys work when a world has focus.</p>:<><p className="record-voice">TILE {selected%data.width}, {Math.floor(selected/data.width)} / TICK {format(tick)}</p><div className="cell80-inspection-pair">{inspection.map((organisms,i)=><div key={i}><strong>{data.histories[i].label}</strong><p>{foodAt(data.histories[i].frames[index],selected)?"40 food":"No food"} · {organisms.length} organisms</p>{organisms.length>0&&<ul>{organisms.slice(0,8).map(o=><li key={o[0]}>#{o[0]} · {o[3]?"predator":"grazer"} · energy {o[2]} · threshold {o[5]} · program {o[4]} ({data.promoters[o[4]]}){o[6]?" · 2231’s lineage":""}</li>)}</ul>}{organisms.length>8&&<p>{organisms.length-8} more organisms share this tile. All are included in the download.</p>}</div>)}</div></>}</div>
    <figcaption className="cell80-caption">{kind==="lineage"?"A fresh replay of the historical library reproduces the recorded birth and plurality event. Both worlds use the same clock. Frames are sampled every two ticks, and every tick from 990–1,200.":"One recorded 2% world, sampled every twenty ticks. Its complete 10,000-tick history hash matches the closure experiment. This shows coexistence; the arms-race screen did not pass."} Squares are actual grid locations; marks group organisms sharing a tile, growing with occupancy. In the lineage view, mixed tiles prioritize program 33, then 37; inspect a tile for every program present. Positions are not interpolated. <a href={`/data/cell80/${kind}-replay.json.gz`} download>Download recorded states ↓</a> · <a href="/data/cell80/replay-provenance.json">Replay provenance ↗</a></figcaption>
  </figure>;
}
