...
import { ecologyActionLabel, ecologyMetric, reminderConditions } from "@/lib/ecology-evidence";
...
export function ContributionStudy() {
 ...
 return <figure className="eco-study">
  ...
  <div className="eco-controls" role="group" aria-label="Supplied posting experience">
   <button aria-pressed={!experienced} aria-controls="contribution-result" onClick={() => setExperienced(false)}>No past example</button>
   <button aria-pressed={experienced} aria-controls="contribution-result" onClick={() => setExperienced(true)}>Show a past example</button>
  </div>
...
export function ReminderStudy() {
 ...
 return <figure className="eco-study">
  ...
  <div className="eco-reminder" id="reminder-result" aria-live="polite" aria-atomic="true"><div><span className="record-voice">{selected ? "THE ADDED NOTE" : "NO ADDED NOTE"}</span><p>{condition.text}</p></div><span className="eco-arrow" aria-hidden="true">→</span><div><span className="record-voice">NEXT ACTION</span><strong data-action={condition.action}>{ecologyActionLabel(condition.action)}</strong></div></div>
