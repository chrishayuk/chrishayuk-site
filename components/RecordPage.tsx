...
import { NotebookEdition, NotebookSupport } from "@chrishayuk/hause/components/NotebookEdition";
import { NotebookReturn } from "./NotebookReturn";
import { RecoveryNotebook } from "./RecoveryNotebook";
import { WorldRemembersNotebook } from "./WorldRemembersNotebook";
import { EcologySeriesUpdate } from "./EcologySeriesUpdate";
...
import { DefaultNotebook } from "./NotebookEntry";
...
import { MachineRecognitionNotebook } from "./MachineRecognitionNotebook";
...
export function RecordPage({ record, canonicalPath, frozen = false }: { record: PublicationRecord; canonicalPath?: string; frozen?: boolean }) {
 ...
 const notebookEdition = visualEdition && !frozen;
 ...
 const recordBar = <div className="record-bar record-voice"><span>{record.id}</span><span>{record.status || record.collection}</span><span>RECORDED {record.created}</span>{brief && modified && <span>PAGE UPDATED <time dateTime={modified}>{modified}</time></span>}<span>{record.publication.toUpperCase()} · V{record.version}</span><span className="record-verbs"><a href="#cite">{citation ? "CITE" : "REFERENCE DRAFT"} ↓</a>{capture && <a href={capture.url} rel="nofollow" title={`Independently archived ${capture.date} by the Internet Archive`}>ARCHIVE ↗</a>}<a href="#follow">FOLLOW ↓</a></span></div>;
 const apparatus = <div className="record-apparatus">{notebookEdition && <div className="notebook-afterword">{brief && <MachineBrief id={record.id}/>}<FieldNotes label="About this note" detail="RECORD +"><p className="record-summary">{record.abstract}</p>{recordBar}</FieldNotes>{journeyPart > 0 && <nav className="cell80-series-marker record-voice" aria-label="Cell80 sequence"><Link href="/thread/cell80">CELL80 / THE EXPERIMENTS ↗</Link><span aria-current="page">NOTE 0{journeyPart} OF 06</span></nav>}</div>}<ThreadNavigation id={record.id}/><PublicationHistory id={record.id} version={record.version}/><section className="sources"><h2>SOURCES & PROVENANCE</h2><ul>{evidenceSources.map((s,i) => <li key={i} id={`source-${i+1}`}>{s.url ? <a href={s.url}>{s.title} ↗</a> : <span>{s.title}</span>}{s.note && <p>{s.note}</p>}{s.preserved && <p className="record-voice"><a href={s.preserved.url}>PRESERVED COPY ↗</a> · CAPTURED {s.preserved.date}</p>}</li>)}</ul><p className="record-voice">AUTHOR / {record.authors.join(", ")} · VERSION / {record.version}</p></section>{related.length>0 && <section className="related-records"><h2>FOLLOW THE RECORD</h2>{related.map(r => <Link key={r.id} href={recordPath(r)}>{r.title}<span>↗</span></Link>)}</section>}{citation ? <><Provenance record={citation} citeHref="#cite"/><Citation record={citation}/></> : <Reference formats={referenceFormats(record)} draft id={record.id}/>}{record.kind==="notebook" && isListed(record) && <NotebookShare record={record}/>}{notebookEdition && <nav className="notebook-return-end" aria-label="Return to notebook after reading"><NotebookReturn href={notebookCollectionHref(notebookAppearance(record.id)["data-notebook-palette"])}/></nav>}<Follow/></div>;
 return <PublicationRoot {...(notebookEdition ? notebookAppearance(record.id) : {})} id="main" data-notebook-page={notebookEdition ? recordPath(record) : undefined} className={`publication-main${notebookEdition ? " notebook-reading-page" : ""}${journeyPart ? " cell80-record" : ""}${opening || agentEdition ? " notebook-record" : ""}${visualEdition ? " visual-notebook-record" : ""}${record.id === "N-AUTHORITY" ? " authority-record" : ""}${addressEdition ? " address-build-record" : ""}${agentEdition ? " agent-attribution-record" : ""}${[...ecologyIds, "N-MACHINE-RECOGNITION", "N-MACHINE-PEER", "N-MACHINE-DISCOVERY", "N-MACHINE-CAPABILITY", "N-MACHINE-MOTIVATION", "N-MACHINE-TASK", "N-MACHINE-VISIT", "N-MACHINE-PERMISSION", "N-MACHINE-SELF-READ"].includes(record.id) ? " machine-visit-record" : ""}`}><JsonLd data={structured}/>{notebookEdition && <nav className="notebook-return-bar" aria-label="Return to notebook"><NotebookReturn href={notebookCollectionHref(notebookAppearance(record.id)["data-notebook-palette"])}/></nav>}{codexEdition ? null : agentEdition ? <AgentAttributionHero record={record} citation={Boolean(citation)} captureUrl={capture?.url}/> : <header className="record-header"><nav className="breadcrumbs record-voice" aria-label="Breadcrumb"><Link href="/">CHRIS HAY</Link><span>/</span><Link href={record.kind === "work" ? "/systems" : `/${sectionFor(record)}`}>{record.kind === "work" ? "SYSTEMS" : sectionFor(record).toUpperCase()}</Link>{!brief && !notebookEdition && <span>/ {record.id}</span>}{notebookEdition && <span className="notebook-state">/ {record.publication === "published" ? "PUBLISHED" : "WORKING NOTE"}</span>}</nav><div className={opening ? "notebook-opening" : undefined}><div><h1 id={addressEdition ? "act-1" : undefined} className={notebookEdition ? "notebook-journey-title" : undefined} style={notebookEdition ? {viewTransitionName: `notebook-${record.id.toLowerCase()}`} : undefined}>{record.title}</h1><p className="dek">{record.dek}</p>{opening && <div className="notebook-entrances"><a className="text-link notebook-read" href="#thread">FOLLOW THE QUESTION ↓</a>{demoLink && <a className="text-link notebook-demo-link" href={demoLink.href}>TRY THE INTERACTIVE STUDY ↗</a>}</div>}</div>{opening && <div className="notebook-opening-media">{addressEdition ? <AddressDepth/> : <Acts acts={[record.body[0]]} anchored priority/>}</div>}</div>{record.kind === "work" && <p className="work-endorsement record-voice">A SYSTEM BY <Link href="/about#the-house">CHRIS HAY ↗</Link></p>}{record.visibility === "unlisted" && <aside className="unlisted-banner" role="note"><p className="record-voice">UNLISTED PREVIEW · NOT PUBLISHED</p>{record.id === "N-MACHINE-MOTIVATION" ? <p>A working notebook edition for review.</p> : <p>This record has a working URL so it can be reviewed in place. It is linked from no index, excluded from the catalogue, the knowledge graph, Ask, the feeds and the sitemap, and marked noindex for search engines. Nothing here is published, and its research claims are not established.</p>}</aside>}{!notebookEdition && (visualEdition ? <details className="notebook-synopsis"><summary className="record-voice">ABOUT THIS NOTE +</summary><p className="record-summary">{record.abstract}</p></details> : <p className="record-summary">{record.abstract}</p>)}{!notebookEdition && recordBar}</header>}
 {notebookEdition && !codexEdition && <div className="notebook-byline"><span>By {record.authors.join(", ")}</span><span>Recorded <time dateTime={record.created}>{record.created}</time></span><span>{record.publication === "published" ? "Published" : "Working note"}</span></div>}
 ...
 {!frozen && <EcologySeriesUpdate id={record.id}/>}
 <div className="record-body" id="thread">{frozen ? <Acts acts={record.body} anchored staticRefusals frozen/> : record.id === "N-ECOLOGY-RECOVERY" ? <RecoveryNotebook/> : record.id === "N-ECOLOGY-WORLD-REMEMBERS" ? <WorldRemembersNotebook/> : record.id === "N-MACHINE-PEER" ? <MachinePeerNotebook record={record}/> : record.id === "N-ECOLOGY-INHERITANCE" ? <InheritanceNotebook record={record}/> : ecologyIds.includes(record.id) ? <EcologyNotebook record={record}/> : record.id === "N-MACHINE-RECOGNITION" ? <MachineRecognitionNotebook record={record}/> : record.id === "N-MACHINE-DISCOVERY" ? <MachineDiscoveryNotebook record={record}/> : record.id === "N-MACHINE-CAPABILITY" ? <MachineCapabilityNotebook record={record}/> : record.id === "N-MACHINE-MOTIVATION" ? <MachineMotivationNotebook record={record}/> : record.id === "N-MACHINE-TASK" ? <MachineTaskNotebook record={record}/> : record.id === "N-MACHINE-SELF-READ" ? <MachineSelfReadNotebook record={record}/> : record.id === "N-MACHINE-PERMISSION" ? <MachinePermissionNotebook record={record}/> : record.id === "N-MACHINE-VISIT" ? <MachineVisitNotebook record={record}/> : record.id === "N-CELL80-BOUND" ? <Cell80BoundNotebook record={record}/> : furtherPart ? <Cell80FurtherNotebook record={record}/> : ecologyPart ? <Cell80Notebook record={record}/> : agentEdition ? <AgentAttributionNotebook acts={record.body}/> : addressEdition ? <AddressBuildNotebook/> : record.id === "N-EXHIBITION" ? <ExhibitionNotebook acts={record.body}/> : record.id === "N-AUTHORITY" ? <AuthorityNotebook acts={record.body}/> : record.id === "N-MAP" ? <MapNotebook acts={record.body}/> : record.id === "N-STATE" ? <StateNotebook acts={record.body}/> : record.id === "N-ADDRESS" ? <AddressNotebook acts={record.body}/> : ["N-CONTEXT", "N-OPERATOR"].includes(record.id) ? <ShortNotebook id={record.id} acts={record.body}/> : codexEdition ? <DefaultNotebook record={record}/> : <Acts acts={opening ? record.body.slice(1) : record.body} anchored offset={opening ? 1 : 0}/> }{record.slug === "mcp-cli" && <section className="sources" id="chuk"><h2>CHUK / TOOL INFRASTRUCTURE</h2><p>The related projects remain available through <a href="https://github.com/chrishayuk">Chris’s software archive ↗</a>.</p></section>}</div>
 {codexEdition ? <div className="codex-record-apparatus"><NotebookSupport label="Sources, citation & publication record">{apparatus}</NotebookSupport></div> : apparatus}</PublicationRoot>;
}
