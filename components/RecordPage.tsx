...
export function RecordPage({ record }: { record: PublicationRecord }) {
 ...
 return <main id="main" className="publication-main"><JsonLd data={structured}/><header className="record-header"><nav className="breadcrumbs record-voice" aria-label="Breadcrumb"><Link href="/">CHRIS HAY</Link><span>/</span><Link href={record.kind === "work" ? "/systems" : `/${sectionFor(record)}`}>{record.kind === "work" ? "SYSTEMS" : sectionFor(record).toUpperCase()}</Link><span>/ {record.id}</span></nav><h1>{record.title}</h1><p className="dek">{record.dek}</p>{record.kind === "work" && <p className="work-endorsement record-voice">A SYSTEM BY <Link href="/about#the-house">CHRIS HAY ↗</Link></p>}<p className="record-summary">{record.abstract}</p><div className="record-bar record-voice"><span>{record.id}</span><span>{record.status || record.collection}</span><span>RECORDED {record.created}</span><span>{record.publication.toUpperCase()} · V{record.version}</span><a href="#cite">{citation ? "CITE" : "REFERENCE DRAFT"} ↓</a></div></header>
 ...
 <div className="record-body" id="thread"><Acts acts={record.body} anchored/>{record.slug === "mcp-cli" && <section className="sources" id="chuk"><h2>CHUK / TOOL INFRASTRUCTURE</h2><p>The related projects remain available through <a href="https://github.com/chrishayuk">Chris’s software archive ↗</a>.</p></section>}</div>
 <div className="record-apparatus"><section className="sources"><h2>SOURCES & PROVENANCE</h2><ul>{record.sources.map((s,i) => <li key={i} id={`source-${i+1}`}>{s.url ? <a href={s.url}>{s.title} ↗</a> : <span>{s.title}</span>}{s.note && <p>{s.note}</p>}</li>)}</ul><p className="record-voice">AUTHOR / CHRIS HAY · VERSION / {record.version}</p></section>{related.length>0 && <section className="related-records"><h2>FOLLOW THE RECORD</h2>{related.map(r => <Link key={r.id} href={recordPath(r)}>{r.title}<span>↗</span></Link>)}</section>}{citation ? <><Provenance record={citation} citeHref="#cite"/><Citation record={citation}/></> : <Reference formats={referenceFormats(record)} draft id={record.id}/>}</div></main>;
}
