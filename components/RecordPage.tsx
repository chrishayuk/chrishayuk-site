...
export function RecordPage({ record }: { record: PublicationRecord }) {
 ...
 return <main id="main" className="publication-main"><JsonLd data={structured}/><header className="record-header"><nav className="breadcrumbs record-voice" aria-label="Breadcrumb"><Link href="/">CHRIS HAY</Link><span>/</span><Link href={`/${sectionFor(record)}`}>{sectionFor(record).toUpperCase()}</Link><span>/ {record.id}</span></nav><h1>{record.title}</h1><p className="dek">{record.dek}</p>{record.kind === "work" && <p className="work-endorsement record-voice">A WORK BY <Link href="/about#the-house">CHRIS HAY ↗</Link></p>}<p className="record-summary">{record.abstract}</p><div className="record-bar record-voice"><span>{record.id}</span><span>{record.status || record.collection}</span><span>RECORDED {record.created}</span><span>{record.publication.toUpperCase()} · V{record.version}</span><a href="#cite">{citation ? "CITE" : "REFERENCE DRAFT"} ↓</a></div></header>
