...
export function RecordPage({ record, canonicalPath }: { record: PublicationRecord; canonicalPath?: string }) {
 ...
 const structured = {
  ...
  ...(record.sources.some(source => source.url) ? { citation: record.sources.filter(source => source.url).map(source => ({ "@type": "CreativeWork", name: source.title, url: source.url })) } : {}),
