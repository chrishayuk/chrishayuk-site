...
export function RecordPage({ record }: { record: PublicationRecord }) {
 ...
 <div className="record-body" id="thread">{agentEdition ? <AgentAttributionNotebook acts={record.body}/> : addressEdition ? <AddressBuildNotebook/> : record.id === "N-EXHIBITION" ? <ExhibitionNotebook acts={record.body}/> : record.id === "N-AUTHORITY" ? <AuthorityNotebook acts={record.body}/> : record.id === "N-MAP" ? <MapNotebook acts={record.body}/> : record.id === "N-STATE" ? <StateNotebook acts={record.body}/> : record.id === "N-ADDRESS" ? <AddressNotebook acts={record.body}/> : ["N-CONTEXT", "N-OPERATOR"].includes(record.id) ? <ShortNotebook id={record.id} acts={record.body}/> : <Acts acts={opening ? record.body.slice(1) : record.body} anchored offset={opening ? 1 : 0}/> }{record.slug === "mcp-cli" && <section className="sources" id="chuk"><h2>CHUK / TOOL INFRASTRUCTURE</h2><p>The related projects remain available through <a href="https://github.com/chrishayuk">Chris’s software archive ↗</a>.</p></section>}</div>
