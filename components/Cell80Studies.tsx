...
export function Cell80Dependence() {
  ...
  return <figure className="cell80-dependence"><figcaption className="record-voice">EX-10 / FOUR VERSIONS · TOTAL BIRTHS IN EACH WORLD</figcaption>
    <div className="cell80-controls" role="group" aria-label="Choose a sequence of changes">{["Offspring energy share", "Reproduction setting"].map((label, i) => <button type="button" key={label} aria-pressed={candidate===i} onClick={() => setCandidate(i)}>{label}</button>)}</div>
    ...
    <p className="cell80-caption">Sequence {candidate === 0 ? "597 → 638" : "4964 → 4975"} · seed {result.seed}. The four bars share a scale, which adjusts for each world. Each version starts a whole population in a fresh world. These are total births across that population, rather than one organism’s offspring.</p>
