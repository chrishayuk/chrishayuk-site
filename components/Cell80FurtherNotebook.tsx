...
{ label: "Discovery and persistence", children: <><StudyRoom label="DISCOVERY → PERSISTENCE" title={<>It appeared.<br/><em>Sometimes it stayed.</em></>} description="A working program can be born and disappear. I ran ten worlds per condition to separate discovering the capability from keeping it common.">
...
function InheritedHistory() : NotebookChapter[] {
  return [
{ label: "The descendants", kind: 'operate', children: <><StudyRoom id="cell80-study" label="EX-13 / FOLLOW ONE ORGANISM’S FAMILY" title={<>Passed on.<br/><em>Then lost.</em></>} tone="dark" description="In a digital world, I tested a new food-processing program and a later change that increased food intake. Then I followed one organism’s descendants to see how long they kept both changes.">
...
{ label: "Before transmission", children: <><StudyRoom label="BEFORE TRANSMISSION / EX-12" title={<>A useful opportunity.<br/><em>No offspring.</em></>}>
...
{ label: "A chance to reproduce", children: <><StudyRoom label="BACK TO THE DESCENDANT / EX-13" title={<>Six wins.<br/><em>Four empty futures.</em></>} tone="accent" description="I took one descendant and replayed ten possible futures with neither change, each change alone, and both. The test asked whether the later intake change still depended on the earlier food-processing capability.">
...
{ label: "The next question", children: <><StudyRoom label="THE NEXT QUESTION IN THIS RECORD / EX-15" title={<>One history.<br/><em>Can it happen again?</em></>}>
...
export function Cell80FurtherNotebook({record}:{record:PublicationRecord}) {
  return <NotebookEntry record={record} className="cell80-notebook cinematic-notebook" introduction={<p className="cell80-reading-key">Dotted terms have short explanations.</p>} chapters={[...(cell80FurtherPart(record.id) === 1 ? Barrier() : InheritedHistory()), { label: "Notes & connections", kind: 'evidence', children: <><Cell80Connection id={record.id}/>
...
<NotebookNavigation><Cell80Journey id={record.id}/><nav className="cell80-entry-nav record-voice" aria-label="Explore this note"><a href="#cell80-study">ENTER THE STUDY ↑</a><a href="#cell80-record">READ THE FULL NOTE ↑</a><Link href="/thread/cell80#further-notes">ALL SIX NOTES ↗</Link></nav></NotebookNavigation></> }]}/>;
}
