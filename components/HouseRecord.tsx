...
export function HouseRecord(){return <section id="the-house" className="house-record" aria-labelledby="house-record-title">
  ...
  <div className="house-works">{HOUSE_WORK.map(w=><article key={w.id}><div className="house-work-identity"><span className="record-voice">{w.field}</span><h3><Link href={w.path}>{w.name} ↗</Link></h3><span className="record-voice">A SYSTEM BY CHRIS HAY</span></div><div><p>{w.text}</p><Link href={w.path} className="text-link">FOLLOW THE WORK ↗</Link></div></article>)}</div>
