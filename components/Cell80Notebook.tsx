...
{ label: "Replay the history", children: <><StudyRoom label="02 / BEFORE THE INTERVENTION" title={<>First, make history<br/><em>repeat itself.</em></>} description="The replay test came before the causal claim. Without identical histories, I could not isolate what the intervention changed.">
...
{ label: "Follow the lineage", children: <><StudyRoom label="03 / FOLLOW THE LINEAGE" title={<>A change became common.<br/><em>Which birth mattered?</em></>} tone="accent" description="The child inherited two changes: a different reproduction program and a different numerical setting. I kept the new setting in both histories, so only the program differed.">
      <Cell80Replay/>
...
{ label: "Did it help?", children: <><StudyRoom label="04 / THE TEST CAN DISAPPOINT YOU" title={<>New code appeared.<br/><em>Did it help?</em></>}>
...
{ label: "The limits", children: <><StudyRoom label="04 / KEEP THE LIMIT IN VIEW" title={<>The world continues.<br/><em>The signal is absent.</em></>}>
...
function InventionNotebook() : NotebookChapter[] {
  return [
{ label: "Does it help?", kind: 'operate', children: <><StudyRoom id="cell80-study" label="01 / DOES IT HELP? · EX-10" title={<>A change happened.<br/><em>Eight of them helped.</em></>} tone="dark" description="In this digital world, grazers inherit programs that control their movement. I combined existing programs and replayed 108 births with the new program kept or undone. Then I counted each changed organism’s own offspring.">
...
{ label: "Novelty", children: <><StudyRoom label="02 / NOVELTY" title={<>Different code.<br/><em>Often the same decision.</em></>} description="Five of the eight helpful combinations made the same movement choices as a program already in the library, for every possible food reading in this world."><Cell80Novelty/><div className="cell80-prose"><p>Three helpful combinations made different choices in this test. All the combinations were prepared before the world ran. Here, evolution inherited prepared programs; it did not assemble them at birth.</p></div></StudyRoom></> },
{ label: "Succession", children: <><StudyRoom label="03 / SUCCESSION" title={<>It helped.<br/><em>Then something else helped.</em></>} tone="accent" description="Next I followed descendants that kept the new movement program. I tested later changes to their reproduction settings, again by keeping or undoing each change at birth.">
...
export function Cell80Notebook({ record }: { record: PublicationRecord }) {
  ...
  return <NotebookEntry record={record} className="cell80-notebook cinematic-notebook" introduction={<p className="cell80-reading-key">Dotted terms have short explanations.</p>} chapters={[...(part === 1 ? ReplayNotebook() : part === 2 ? SurvivalNotebook() : InventionNotebook()), { label: "Notes & connections", kind: 'evidence', children: <><Cell80Connection id={record.id}/>
<PriorWork part={part}/>
<ReadingRecord record={record}/>
<NotebookNavigation><Cell80Journey id={record.id}/><nav className="cell80-entry-nav record-voice" aria-label="Explore this note"><a href="#cell80-study">ENTER THE STUDY ↑</a><a href="#cell80-record">READ THE FULL NOTE ↑</a><Link href="/thread/cell80">ALL SIX NOTES ↗</Link></nav></NotebookNavigation></> }]}/>;
}
