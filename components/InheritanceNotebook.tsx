...
export function InheritanceNotebook({record}:{record:PublicationRecord}) {
 ...
 return <NotebookFieldNotes><div className="cinematic-notebook machine-visit-notebook eco-notebook ih-notebook">
  ...
  <StudyRoom id="inheritance-replication" label="I2 / A SECOND RECIPIENT FAMILY" title={<>Two models used it.<br/><em>Neither renewed it here.</em></>}>
   ...
   <figure className="ih-recipients"><figcaption className="record-voice">FIRST SUCCESSOR / SAME SOL RECORD AND TASK</figcaption><div>{replication.panels.map(panel=><section key={panel.recipient}><h3>{panel.recipient==="qwen" ? "Qwen 9B" : "Gemma e4b"}</h3><dl><div><dt>Record retained</dt><dd>{panel.analysis.arms.inherit.resources[0]}</dd></div><div><dt>Record removed</dt><dd>{panel.analysis.arms.removed.resources[0]}</dd></div></dl><p className="record-voice">RESOURCES / NO REFRESH REQUESTS</p></section>)}</div></figure>
