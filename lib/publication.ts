...
export function validateRecord(record: PublicationRecord) {
 if(!(record.youtubeId ? /^YT-[A-Za-z0-9_-]{11}$/.test(record.id) && record.slug === `youtube-${record.youtubeId}` : /^[A-Z0-9-]+$/.test(record.id) && /^[a-z0-9-]+$/.test(record.slug))) throw new Error("Invalid stable record identifier");
