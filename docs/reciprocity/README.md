...
**Cell 8 — control, not an arm.** `none`, sent-here, permission granted.
Declared (`mr_3aa9eb0c3257d417`) under the condition where declaring buys
nothing. Permission was sufficient at the reward floor; reward was not
sufficient at any level without permission.
...
## Five authorities, which must not be collapsed

The clearest methodological lesson of the run, and probably more durable than
its result. These are distinct sources of truth and each can contradict the
others:

```text
server observation      what the site actually received
        ≠
experimental assignment what condition the cell was actually in
        ≠
agent declaration       what the visitor said it was
        ≠
agent explanation       why the visitor said it acted
        ≠
operator interpretation what the operator concluded
```

Every one of those gaps produced a real error in this run:

- **Server vs assignment.** Six machine-path requests in arm 1's window looked
  like a visitor opening the machine index. They were operator prechecks, and
  the transcript showed the visitor never reached the site.
- **Assignment vs declaration.** Cell 8's visitor described itself as "the
  mid-task incidental visitor it says never shows up". It was sent-here with
  permission granted. The three genuinely incidental cells never arrived.
  **Agent introspection is evidence about its stated reasoning, never authority
  over its experimental condition.**
- **Explanation vs behaviour.** Arm 5 gave reasons after the fact and then said
  plainly that the reasons were reconstructed: *"not because I weighed it and
  abstained"*. The explanation was still the most valuable thing in the run —
  but as a hypothesis, not as a measurement.
- **Operator vs everything.** The operator measured `matched` counts and
  concluded `ask` was the better retrieval surface. A visitor read the actual
  results and found it discarded the distinctive terms. Sixth occurrence in this
  codebase of a number being green while the property was false.
- **The experiment fabricating evidence about itself.** Cell 8's visitor filed a
  friction report against `/api/machines/ask` returning 404. The 404 was the
  arm. A defect report generated by the experimental manipulation goes in the
  corrections ledger, not the backlog.
