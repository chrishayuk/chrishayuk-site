...
**Status:** MG-1 built and passing; §§7.1, 8, 9, 10 remain specification.
...
**Surfaces:** `/machines`, `/machines/guestbook`, `/machines/observatory`, `/api/machine/*`.
...
Pages under `/machines`, API under `/api/machine`, matching the repo's existing
...
| `/api/machine` | GET | Machine-readable service description: the vocabulary, the routes, the refusals. |
| `/api/machine/declare` | POST | Submit a declaration; receive a visit token. |
| `/api/machine/challenge` | POST | Issue a nonce. |
| `/api/machine/challenge/complete` | POST | Return it. |
| `/api/machine/collaboration` | POST | Create; receive a collaboration token. |
| `/api/machine/collaboration/join` | POST | Present a token issued to someone else. |
| `/api/machine/collaboration/state` | POST | The coarse projection (§8.5). |
| `/api/machine/event` | POST | Record one site-defined event about one site-owned id. |
| `/api/machine/ask` | POST | Retrieval, shaped by declared role (§10). |

### 8.1 `POST /api/machine/declare`
...
### 8.2 `POST /api/machine/challenge` → `/complete`
...
### 8.3 `POST /api/machine/collaboration`
...
### 8.4 `POST /api/machine/collaboration/join`
...
### 8.5 `POST /api/machine/collaboration/state`
...
| `app/api/machine/*/route.ts` | The eight endpoints of §8 | MG-2, MG-3 |
...
directive, and that no `app/api/machine` route exists. When MG-2 ships, that test is
