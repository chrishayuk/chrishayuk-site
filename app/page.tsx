...
export default function Home() {
 ...
 return <main id="main" className="homepage editorial-home">
  ...
  <section id="from-the-notebook" className="notebook-scene notebook-feature" data-scene="latest-notebook" data-hause-act="connection" aria-labelledby="notebook-feature-heading">
   ...
   {cell80Part(notebook.id) > 0 && <nav className="cell80-home-sequence" aria-label="Read all five Cell80 Notebook entries">
    <div className="cell80-home-sequence-heading"><Kicker>CELL80 / FIVE EXPERIMENT NOTES</Kicker><p>Replay a birth. Follow a new capability. See what survives.</p></div>
    <ol>{ecologySteps.map((step,i)=><li key={step.id}><Link href={step.url}><span className="record-voice">0{i+1} / {step.label}</span><h3>{step.title}</h3><p>{ecologyIntroductions[i]}</p><span className="record-voice">OPEN THE NOTE ↗</span></Link></li>)}{cell80FurtherRecords.map((note,i)=><li key={note.id} className="cell80-home-later-note"><Link href={recordPath(note)}><Cell80FurtherCard part={i+1}/><span className="record-voice">0{i+4} / {i===0?"A NEW CAPABILITY":"FOLLOW THE DESCENDANTS"}</span><h3>{note.title}</h3><p>{i===0?"Play forty worlds to see when a program for new food appears—and whether it stays.":"Follow one family through time, then compare a hundred possible futures for an inherited change."}</p><span className="record-voice">{i===0?"PLAY THE EXPERIMENT":"FOLLOW THE FAMILY"} ↗</span></Link></li>)}</ol>
   </nav>}
  </section>
