...
export async function generateMetadata({params}:Props):Promise<Metadata>{const{section,slug}=await params;const r=getRecord(slug);if(section==="film"&&slug==="mixture-of-experts")return pageMetadata("Mixture of Experts",panelistIntroduction,"/film/mixture-of-experts");if(!r)return{title:"Record not found"};const meta=pageMetadata(r.title,`${r.abstract} ${socialState(r)}`,recordPath(r),r.kind==="notebook"&&r.visibility!=="unlisted"?socialImage(r):undefined,citationRecord(r)||undefined);return r.visibility==="unlisted"?{...meta,robots:{index:false,follow:false}}:meta;}
