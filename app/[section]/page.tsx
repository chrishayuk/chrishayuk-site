...
export default async function IndexPage({params,searchParams}:PageProps) {
 ...
 if(section==="colophon") return <main id="main" className="prose-page"><p className="kicker record-voice">PUBLICATION NOTES / 0.1</p><h1>The colophon.</h1><p>CHRIS HAY is a house for ideas, systems and objects: one practice across research, engineering, design and film. This publication is its primary record. CHRISHAYUK is the web address and handle.</p><h2>An evolving publication</h2><p>The site brings together software, films and research as the work develops. Each record carries its own status, sources and dates. Notebook drafts remain drafts until individually reviewed and published.</p><h2>Composed with HAUSE</h2><p>Real HAUSE semantic forms render the record. The publication adds its own photographic sequences, responsive media and a shared motion coordinator. Fraunces, Inter and Geist Mono supply the editorial, text and record voices.</p><p><a href="https://hause.design/">The specimen book ↗</a></p><h2>Rights and sources</h2><p>Selected IBM episodes use their official YouTube players and posters. IBM remains the producer and host of the original footage; Chris appears as a participant. The collection leads with his latest verified appearance and the most-viewed episode in the official playlist snapshot. Earlier editorial appearance records remain available.</p><p>The typographic social image was generated for this publication. Research notes retain source descriptions and exact draft status.</p><details><summary>The media and production register</summary><p>The register includes available media and planned photography or film. Some planned material is not used on the current homepage. Where a page shows a production slot, it is labelled; it is not a photograph of Chris or his work.</p><ul className="production-list">{media.map(m=><li key={m.id}>{m.title}<span>{m.state === "ready" ? "AVAILABLE" : "PLANNED"}</span></li>)}</ul><p>Illustrative E25 findings from the original brief have not been published as experimental facts.</p></details><h2>Publishing the record</h2><p>Published records enter the feeds. The machine-readable index also includes source-attributed YouTube catalogue records. Draft references omit a publication date. Material revisions will retain their earlier snapshots and citations.</p><p><a href="/api/records">Public record index</a> · <a href="/rss.xml">RSS</a> · <a href="/feed.json">JSON Feed</a></p></main>;
 if(section==="accessibility") return <main id="main" className="prose-page"><p className="kicker record-voice">READING THE PUBLICATION</p><h1>At your pace.</h1><h2>Motion is optional</h2><p>Use the motion control in the header to pause automatic movement. Reduced-motion and supported reduced-data preferences start with still compositions. Sound never starts automatically.</p><h2>A readable record</h2><p>Headings, descriptions, notebook text and sources are server-rendered. The story does not require interpreting an animation. Keyboard navigation and explicit controls accompany interactive forms.</p><h2>Film and transcripts</h2><p>The current film selection links to IBM’s original episode pages, where the films and transcripts can be read or watched. Media rights determine whether a transcript can also be hosted here.</p><h2>Current review status</h2><p>This production edition has not completed a browser accessibility audit. The media shoot and final image contrast remain part of the ongoing production review.</p><Link href="/about" className="text-link">CONTACT CHRIS ↗</Link></main>;
