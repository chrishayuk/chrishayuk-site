...
export default function Page() {
  ...
  const cards: Record<string, typeof MachineVisitCard>={"N-MACHINE-VISIT":MachineVisitCard,"N-MACHINE-PERMISSION":MachinePermissionCard,"N-MACHINE-SELF-READ":MachineSelfReadCard,"N-MACHINE-TASK":MachineTaskCard,"N-MACHINE-MOTIVATION":MachineMotivationCard,"N-MACHINE-CAPABILITY":MachineCapabilityCard};
  return <main id="main" className="thread-page machine-thread">
    ...
    <header className="thread-intro"><p className="kicker record-voice">CHRIS HAY / {legibility.subject.toUpperCase()}</p><h1>The website asks.<br/><em>Whose instruction counts?</em></h1><p className="dek">{thread.abstract}</p><p className="thread-context">{thread.context}</p><div className="record-bar record-voice"><a href="#field-map">EXPLORE THE FIELD MAP ↓</a><a href="#step-1">READ THE SIX VISITS ↓</a><Link href="/notebook/the-tool-was-not-the-problem">LATEST / CAPABILITY & DISCOVERY ↗</Link><a href="#instruments">EXPLORE THE INSTRUMENTS ↓</a></div></header>
    <MachineFieldMap/>
    <div className="machine-thread-latest"><Link href="/notebook/the-tool-was-not-the-problem" aria-label="Read the latest machine experiment"><MachineCapabilityCard/></Link></div>
