...
import { MachineInstruments, MachineOutcomeStrip } from "@/components/MachineProgramme";
...
export default function Page() {
  ...
  const cards: Record<string, typeof MachineVisitCard>={"N-MACHINE-VISIT":MachineVisitCard,"N-MACHINE-PERMISSION":MachinePermissionCard,"N-MACHINE-SELF-READ":MachineSelfReadCard,"N-MACHINE-TASK":MachineTaskCard};
  return <main id="main" className="thread-page machine-thread">
    <JsonLd data={{"@context":"https://schema.org","@type":"CollectionPage",name:thread.title,url:`${SITE}${thread.path}`,description:thread.abstract,mainEntity:{"@type":"ItemList",itemListOrder:"https://schema.org/ItemListOrderAscending",numberOfItems:steps.length,itemListElement:steps.map((step,i)=>({"@type":"ListItem",position:i+1,name:step.title,url:`${SITE}${step.url}`}))}}}/>
    <header className="thread-intro"><p className="kicker record-voice">CHRIS HAY / THE NOTEBOOK / MACHINE EXPERIMENTS</p><h1>The website asks.<br/><em>Whose instruction counts?</em></h1><p className="dek">{thread.abstract}</p><p className="thread-context">{thread.context}</p><div className="record-bar record-voice"><a href="#step-1">BEGIN WITH THE FIRST VISIT ↓</a><Link href="/notebook/the-page-could-ask-it-couldnt-authorise">LATEST / THE TASK BOUNDARY ↗</Link><a href="#instruments">EXPLORE THE INSTRUMENTS ↓</a></div></header>
    <div className="machine-thread-latest"><MachineOutcomeStrip/></div>
