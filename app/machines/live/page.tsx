...
export default async function Page() {
 ...
 return <main id="main" className="publication-main machine-exhibition">
  ...
  <section className="me-room" aria-labelledby="live-heading">
   <div className="me-section-head"><h2 id="live-heading">The orbit.</h2><p className="record-voice">HOME IS THE CENTRE · MACHINES SITS OUTSIDE EVERY RING</p></div>
   {report.recording ? <MachineLive initialRecent={initialRecent}/> : <p className="me-empty">This deployment keeps no readership counters. The orbit stays dark, without invented arrivals.</p>}
   <p className="me-caption">Real events from the same public counters <Link href="/readership">/readership</Link> already publishes, polled roughly once a minute — the underlying figures still trail live traffic by up to five minutes, the same lag stated there. Shape follows purpose: a user-mediated read draws a line in and lands; an index sweep is a broader arc along one ring; training reads as a sustained wash across the territory it touched, never as individually rendered requests; unidentified automation stays a peripheral flicker with no fixed origin, at Machines. Sharpness follows evidence, separately: verified resolves cleanly, declared stays softer, unknown never resolves into a shape at all. Home’s brightness is the aggregate of what’s actually happened in the last two hours, not a fixed value. No colour is assigned by provider. Machines is drawn apart from the four content rooms because it is a declared interface, not a content page — not because it is hard to find; it has been linked from this site’s own footer since 2026-09-11.</p>
  </section>
