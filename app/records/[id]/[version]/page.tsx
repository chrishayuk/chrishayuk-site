...
export default async function Version({params}:{params:Promise<{id:string;version:string}>}){const{id,version}=await params;const snapshot=getVersion(id,version);if(!snapshot)notFound();return <RecordPage frozen record={snapshot.record} canonicalPath={`/records/${id}/${version}`}/>;}
