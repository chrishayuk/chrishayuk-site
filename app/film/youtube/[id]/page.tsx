...
import {getVideo,videoConcepts,videoWork,transcriptFor,videoReferences,durationLabel,viewLabel,youtube,videoPath} from "@/lib/youtube";
...
export default async function Page({params,searchParams}:Props){
 ...
 <header className="screening-title"><nav className="breadcrumbs record-voice" aria-label="Breadcrumb"><Link href="/film">FILM</Link><span>/</span><Link href="/film/youtube">YOUTUBE</Link></nav><h1>{v.title}</h1><div className="record-bar record-voice"><span>{v.id}</span><span>{durationLabel(v.duration)}</span>{v.published&&<span>PUBLISHED ON YOUTUBE {v.published}</span>}<a href="#cite">CITE ↓</a></div></header>
 <VideoScreening video={v} preview={previewFor(v)} passages={transcript?.passages||[]} initialStart={start}/>
 <div className="screening-details"><section className="film-description"><p className="kicker record-voice">FROM CHRIS / ORIGINAL VIDEO DESCRIPTION</p>{v.description?<p>{v.description}</p>:<p>Chris Hay’s “{v.title}”. This catalogue entry currently contains the channel listing; a full description and transcript have not yet been imported.</p>}<a className="text-link" href={v.url}>WATCH THE ORIGINAL ON YOUTUBE ↗</a></section>
 ...
 <section className="sources"><h2>SOURCE RECORD</h2><p>Created and published by Chris Hay on YouTube. Catalogued {youtube.retrievedAt.slice(0,10)}. {viewLabel(v)} at retrieval{v.viewsApproximate?" (rounded by YouTube)":""}.</p><p>Film publication dates are retained separately from catalogue retrieval dates. Citations below identify the original film.</p><a href={`/api/record/${v.id}`}>MACHINE-READABLE RECORD ↗</a></section>
