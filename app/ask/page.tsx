...
export default async function Page({searchParams}:{searchParams:Promise<{[key:string]:string|string[]|undefined}>}){
 ...
 return <main id="main" className="ask-page"><p className="kicker record-voice">THE CONNECTED RECORD</p><h1>Ask<br/><em>the work.</em></h1><p className="dek">Find the question. Follow the work. Return to the source.</p>
  ...
  <div className="ask-prompts">{["LARQL","FFN graph","operator knows","predictive locality","context reconstructing","MCP","How many episodes?"].map(s=><Link key={s} href={`/ask?q=${encodeURIComponent(s)}`}>{s} ↗</Link>)}</div>
  <p className="ask-coverage record-voice">{coverage.systems} SYSTEMS · {coverage.notebook} NOTEBOOK ENTRIES · {coverage.questions} QUESTIONS · {coverage.films} FILMS · {coverage.chapters} CHAPTERS</p>
