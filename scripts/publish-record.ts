...
const revision = previous.some((s:{record:{id:string}})=>s.record.id===id);
const published=validateRecord({...record,publication:"published",version,published:record.published||date,...(revision?{revised:date}:{})});
