...
for(const n of [1,2,3,4,5,6]) assert.ok(ecologyThread.body.includes(`id="step-${n}"`));
const boundNote=await request('/notebook/can-something-evolve-that-makes-the-next-invention-possible');
assert.equal(boundNote.status,200);
assert.match(boundNote.body,/cell80-control-study/);
assert.match(boundNote.body,/Cell80 experiment journey/);
...
const motivationVersion=await request('/records/N-MACHINE-MOTIVATION/1.0');
