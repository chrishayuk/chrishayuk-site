import {selectArchiveUrls, cdxCapture, mergeFirst} from "./wayback-policy.ts";
import { readFile, writeFile } from "node:fs/promises";
import { archiveUrls } from "../lib/canonical.ts";
import type { Capture } from "../lib/archive.ts";

/**
 * ARCHIVE THE CANONICAL OBJECTS.
 *
 ...
 * Now to capture each canonical URL, then reads the Wayback index back
 * to learn when that URL was FIRST captured, and records the answer in
 * content/archive.json for the provenance surfaces to print.
 *
 * The first capture is the evidence. Everything here exists to record
 * it accurately and then never touch it again:
 *
 * - A capture already on file is never moved forward. If Wayback
 *   reports an earlier one than we hold, we take the earlier one; if
 *   it reports a later one, we keep ours.
 * - A URL is never submitted unless the live site returns 200 for it.
 *   Asking a public archive to preserve a 404 is worse than useless —
 *   it puts a broken copy of your record on someone else's servers.
 * - Nothing is invented. A URL with no capture gets no entry, and the
 *   page that would have shown it shows nothing.
 *
 * Modes:
 *
 *   resolve-only   Ask the CDX index what already exists. Submits
 *                  nothing, needs no credentials, changes nothing
 *                  outside this repository. The default when no
 *                  credentials are set.
 *   submit         Additionally ask Save Page Now for captures that
 *                  are missing or stale. Needs IA_ACCESS_KEY and
 *                  IA_SECRET_KEY, which are Internet Archive S3-style
 *                  keys from https://archive.org/account/s3.php.
 *
 * Usage:
 *   node --experimental-strip-types scripts/archive-wayback.ts [options]
 *
 *   --resolve-only     never submit, even with credentials
 *   --stale-days N     resubmit a URL last captured more than N days
 *                      ago (default 30)
 *   --url URL          archive only this URL; repeatable
 *   --limit N          stop after N submissions in one run
 */

const CDX = "https://web.archive.org/cdx/search/cdx";
const SPN = "https://web.archive.org/save";
const FILE = new URL("../content/archive.json", import.meta.url);
const UA = "chrishayuk.com archive-wayback (+https://chrishayuk.com/colophon)";
...
const daysSince = (value: string) => {
 const iso = `${value.slice(0, 4)}-${value.slice(4, 6)}-${value.slice(6, 8)}T${value.slice(8, 10) || "00"}:${value.slice(10, 12) || "00"}:${value.slice(12, 14) || "00"}Z`;
...
async function savePageNow(url: string, waited = 0): Promise<{ timestamp: string } | null> {
 const body = new URLSearchParams({
  ...
  if_not_archived_within: `${STALE_DAYS}d`,
 });
...
async function serves(url: string): Promise<boolean> {
 try {
  const response = await request(url, { method: "GET", headers: { "User-Agent": UA }, redirect: "manual" });
  return response.status === 200;
 } catch { return false; }
}

const held: Record<string, Capture> = JSON.parse(await readFile(FILE, "utf8"));
const urls = selectArchiveUrls(archiveUrls(), values("url"));
console.log(`${urls.length} canonical URLs · ${SUBMIT ? "submitting" : "resolve-only"}${SUBMIT ? "" : ACCESS ? " (--resolve-only)" : " (no IA credentials)"}`);
